use core::{
    ...
    future::Future,
    pin::Pin,
    task::{Poll, Context}, alloc::Layout,
};

use asyncc::*;
use buddy_system_allocator::LockedHeap;
use config::{PAGE_SIZE, USER_HEAP_SIZE, USER_HEAP_PTR, PAGE_MASK};
/// This mod define `Process`
/// 

use spin::{Lazy, Mutex};
use alloc::{vec::Vec, sync::{Arc, Weak}, boxed::Box};
use crate::{mm::{MM, loader, VMArea, VMFlags}, fs::{File, FDManager}, KernelError, frame_alloc_more};
...
impl Process {
    pub fn idle() -> Self {
        Self {
            ...
            mm: Mutex::new(MM::new(false).unwrap()),
...
impl Future for Process {
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        if self.state.lock().contains(TaskState::ZOMBIE) {
            ...
            let token = self.mm.lock().page_table.token();
